...
2. **One foreground per fill.** A single foreground shared across fills of differing lightness cannot clear a text threshold on all of them. `driver-status` failed at rest for this reason until it was split into a foreground and interaction family per status; each map-marker variant still shares one foreground.
